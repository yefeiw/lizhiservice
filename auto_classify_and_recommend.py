...
def generate_categories(driver, hrefs):
	categories = dict()
	...
	try:
		for href in hrefs:
			print ("href is "+href)
			driver.get(href)
			#need to wait for html loading to complete. Hence waiting for 1 second.
			time.sleep(1)
		...
		return categories
def recommend(categories):


try:
	driver = webdriver.Chrome()
	addresses = retrieve_hrefs(driver)
	categories = generate_categories(driver, addresses)
except:
	logging.error(traceback.print_exc())
finally:
	driver.close()
